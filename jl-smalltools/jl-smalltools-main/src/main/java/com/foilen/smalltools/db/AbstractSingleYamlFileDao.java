...
import com.foilen.smalltools.tools.YamlTools;
import com.foilen.smalltools.trigger.SmoothTrigger;

/**
 * <p>
 * This is a simple DAO that can contain a single entity and it will be persisted to a single yaml file. Whenever {@link #save(Object)} is called, it will request the save of the file, but it won't be
 ...
 *
 * <p>
 * It is doing the same as {@link AbstractSingleJsonFileDao}, but it supports polymorphism because when serializing, the class type is in the YAML.
 * </p>
 *
 * <p>
...
public abstract class AbstractSingleYamlFileDao<T> extends AbstractBasics {
    ...
    protected Runnable saveToFile = () -> {
        ...
        String cachedMd5sum = HashMd5sum.hashString(YamlTools.prettyPrint(cached));
        ...
        try {
            YamlTools.writeToStream(out, cached);
            out.flush();
    ...
    public T load() {
        ...
        try {
            if (cached != null) {
                ...
                return YamlTools.clone(cached);
            }
            if (getFinalFile().exists()) {
                logger.debug("Loading from file");
                String yaml = FileTools.getFileAsString(getFinalFile());
                try {
                    cached = YamlTools.readFromString(yaml, getType());
                } catch (Exception e) {
                    logger.debug("Problem loading file", e);
                    cached = null;
                }
                if (cached == null) {
                ...
                previousMd5sum = HashMd5sum.hashString(yaml);
                return YamlTools.clone(cached);
            } else {
